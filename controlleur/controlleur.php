                        ...
                        case 20: // Nouveau case pour récupérer les livraisons
                            ...
                            case 21: 
                                ...
                                case 22:
                                    ...
                                    case 23:
                                        if (!isset($_POST['idBL'], $_POST['idP'], $_POST['quantity'], $_POST['idBC'], $_POST['unite'])) {
                                        ...
                                        try {
                                            ...
                                            if (count($rep) <= 0) {
                                                $stmt = $gc->getDb()->prepare("INSERT INTO bon_livraison_produit (idBL, idP, quantite, unite, dateadd) VALUES (:idBL, :idP, :quantity, :unite, :dateadd)");
                                                $stmt->execute([':idBL' => $idBL, ':idP' => $idP, ':quantity' => $quantity, ':unite' => $unite, ':dateadd' => $date]);
                                            } else {
                                                $stmt = $gc->getDb()->prepare("UPDATE bon_livraison_produit SET quantite = quantite + :quantity, unite = :unite WHERE idP = :idP AND idBL = :idBL");
                                                $stmt->execute([':quantity' => $quantity, ':unite' => $unite, ':idP' => $idP, ':idBL' => $idBL]);
                                            }
                            ...
                            case 70:
                                ...
                                case 72:
                                    ...
                                    case 73:
                                        ...
                                        $unite = $_POST['unite'];
                                        $date = (new DateTime("now", new DateTimeZone("Africa/Dakar")))->format("Y-m-d H:i:s");
                                        ...
                                        try {
                                            // Quantité sortie
                                            $stmt = $gc->getDb()->prepare("SELECT SUM(quantite) FROM bon_sortie_produit WHERE idP = :idP AND idBS = :idBS");
                                            $stmt->execute([':idP' => $idP, ':idBS' => $idBS]);
                                            $qte_bs = floatval($stmt->fetchColumn() ?? 0);
                                    
                                            // Quantité demandée
                                            $stmt = $gc->getDb()->prepare("SELECT ebp.quantite FROM expression_besoin_produit ebp JOIN bon_sortie bs ON bs.expID = ebp.idEB WHERE ebp.idP = :idP AND bs.idBS = :idBS");
                                            $stmt->execute([':idP' => $idP, ':idBS' => $idBS]);
                                            $qte_eb = floatval($stmt->fetchColumn() ?? 0);
                                    
                                            $reste = $qte_eb - $qte_bs;
                                    
                                            // Vérif stock
                                            $stmt = $gc->getDb()->prepare("SELECT Stock_actuel FROM product WHERE idP = :idP");
                                            $stmt->execute([':idP' => $idP]);
                                            $stockActuel = floatval($stmt->fetchColumn());
                                    
                                            if ($quantity > $stockActuel) {
                                                echo json_encode(['success' => false, 'message' => "Stock insuffisant ($stockActuel)"]);
                                                exit;
                                            }
                                    
                                            if ($reste > 0 && $quantity > $reste) {
                                                echo json_encode(['success' => false, 'message' => "Quantité invalide : dépasse le reste à sortir ($reste)"]);
                                                exit;
                                            }
                                    
                                            if ($reste <= 0 && $quantity > $qte_eb) {
                                                echo json_encode(['success' => false, 'message' => "Quantité invalide : dépasse la quantité demandée ($qte_eb)"]);
                                                exit;
                                            }
                                    
                                            // Existe ?
                                            $stmt = $gc->getDb()->prepare("SELECT COUNT(*) FROM bon_sortie_produit WHERE idP = :idP AND idBS = :idBS");
                                            $stmt->execute([':idP' => $idP, ':idBS' => $idBS]);
                                            $exists = $stmt->fetchColumn() > 0;
                                    
                                            if (!$exists) {
                                                $stmt = $gc->getDb()->prepare("INSERT INTO bon_sortie_produit (idBS, idP, quantite, unite, dateadd) VALUES (:idBS, :idP, :quantity, :unite, :dateadd)");
                                                $stmt->execute([
                                                    ':idBS' => $idBS,
                                                    ':idP' => $idP,
                                                    ':quantity' => $quantity,
                                                ...
                                                $stmt = $gc->getDb()->prepare("UPDATE bon_sortie_produit SET quantite = quantite + :quantity, unite = :unite WHERE idBS = :idBS AND idP = :idP");
                                                $stmt->execute([
                                                    ':quantity' => $quantity,
                                                    ':unite' => $unite,
                                                    ':idBS' => $idBS,
                                                ...
                                                $stmt = $gc->getDb()->prepare("UPDATE bon_sortie_produit SET quantite = :quantity, unite = :unite WHERE idBS = :idBS AND idP = :idP");
                                                $stmt->execute([
                                                    ':quantity' => $quantity,
                                                    ':unite' => $unite,
                                                    ':idBS' => $idBS,
                                        ...
                                        case 74:
                                            try {
                                                ...
                                                // Récupérer l'expression de besoin liée à ce bon
                                                $stmt = $gc->getDb()->prepare("SELECT expID FROM bon_sortie WHERE idBS = :idBS");
                                                $stmt->execute([':idBS' => $idBS]);
                                                $expID = $stmt->fetchColumn();
                                        
                                                if (!$expID) {
                                                    echo json_encode(['success' => false, 'message' => 'Aucune expression de besoin liée.']);
                                                    exit;
                                                }
                                                ...
                                                $sql = "
                                                    SELECT 
                                                        ...
                                                        p.nomproduit,
                                                        p.Stock_actuel,
                                                        ebp.quantite AS quantite_eb,
                                                        COALESCE((
                                                            SELECT SUM(bsp.quantite)
                                                            FROM bon_sortie_produit bsp
                                                            WHERE bsp.idP = ebp.idP AND bsp.idBS = :idBS
                                                        ), 0) AS quantite_sortie,
                                                        (
                                                            SELECT unite 
                                                            FROM bon_sortie_produit 
                                                            WHERE idBS = :idBS AND idP = ebp.idP 
                                                ...
                                                $stmt = $gc->getDb()->prepare($sql);
                                                $stmt->execute([':idBS' => $idBS, ':idEB' => $expID]);
                                                $produits = $stmt->fetchAll(PDO::FETCH_ASSOC);
                                        
                                                foreach ($produits as &$prod) {
                                                    $prod['reste'] = max(0, floatval($prod['quantite_eb']) - floatval($prod['quantite_sortie']));
                                                }
                                        ...
                                        case 76:
                                            try {
                                                if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enregistrerTout'])) {
                                                    ...
                                                    foreach ($idP as $i => $id) {
                                                        $quantite_demandee = floatval($quantite[$i]);
                                        
                                                        // Infos produit
                                                        $stmt = $gc->getDb()->prepare("SELECT nomproduit, Stock_actuel FROM product WHERE idP = :idP");
                                                        $stmt->execute([':idP' => $id]);
                                                        $result = $stmt->fetch(PDO::FETCH_ASSOC);
                                                        $nomProduit = $result['nomproduit'] ?? 'Nom inconnu';
                                                        $stockActuel = $result['Stock_actuel'] ?? 0;
                                        
                                                        // Quantité déjà sortie
                                                        $stmt = $gc->getDb()->prepare("SELECT quantite FROM bon_sortie_produit WHERE idP = :idP AND idBS = :idBS");
                                                        $stmt->execute([':idP' => $id, ':idBS' => $idBS]);
                                                        $qte_bs = $stmt->fetchColumn() ?? 0;
                                        
                                                        // Quantité demandée
                                                        $stmt = $gc->getDb()->prepare("SELECT quantite FROM expression_besoin_produit WHERE idP = :idP AND idEB = :idEB");
                                                        $stmt->execute([':idP' => $id, ':idEB' => $idEB]);
                                                        $qte_eb = $stmt->fetchColumn() ?? 0;
                                                        ...
                                                        if ($reste > 0) {
                                                            ...
                                                            if ($quantite_demandee > $reste) {
                                                                $produitsIncoherents[] = [
                                                                    ...
                                                                    'quantiteSaisie' => $quantite_demandee,
                                                                    'reste' => $reste,
                                                                    'message' => 'Quantité saisie dépasse le reste à sortir'
                                                                ];
                                                                continue;
                                                            }
                                                            ...
                                                            if ($quantite_demandee > $qte_eb) {
                                                                $produitsIncoherents[] = [
                                                                    'nomProduit' => $nomProduit,
                                                                    'quantiteSaisie' => $quantite_demandee,
                                                                    'reste' => $qte_eb,
                                                                    'message' => 'Quantité dépasse la demande totale'
                                                                ];
                                                                continue;
                                                            }
                                        
                                                            $stmt = $gc->getDb()->prepare("UPDATE bon_sortie_produit SET quantite = :quantite, unite = :unite WHERE idBS = :idBS AND idP = :idP");
                                                            $stmt->execute([
                                                                ':quantite' => $quantite_demandee,
                                                                ':unite' => $unite[$i],
                                                                ':idBS' => $idBS,
                                ...
                                case 78:
                                    ...
                                    case 79:
                                        ...
                                        try {
                                            // Récupérer la date d'ajout et quantité actuelle sortie
                                            $stmt = $gc->getDb()->prepare("SELECT dateadd, quantite FROM bon_sortie_produit WHERE idP = :idP AND idBS = :idBS");
                                            $stmt->execute([':idP' => $idP, ':idBS' => $idBS]);
                                            $row = $stmt->fetch(PDO::FETCH_ASSOC);
                                    
                                            if (!$row) {
                                                echo json_encode(['success' => false, 'message' => "Aucun enregistrement trouvé à modifier."]);
                                                exit;
                                            }
                                    
                                            $date = $row['dateadd'];
                                            $qte_sortie_actuelle = floatval($row['quantite']);
                                    
                                            // Récupérer l'expression de besoin liée
                                            $stmt = $gc->getDb()->prepare("SELECT expID FROM bon_sortie WHERE idBS = :idBS");
                                            $stmt->execute([':idBS' => $idBS]);
                                            $expID = $stmt->fetchColumn();
                                            ...
                                            // Quantité demandée
                                            $stmt = $gc->getDb()->prepare("SELECT quantite FROM expression_besoin_produit WHERE idP = :idP AND idEB = :idEB");
                                            $stmt->execute([':idP' => $idP, ':idEB' => $expID]);
                                            $qte_eb = floatval($stmt->fetchColumn());
                                    
                                            // Quantité totale sortie (hors ligne actuelle)
                                            $stmt = $gc->getDb()->prepare("SELECT SUM(quantite) FROM bon_sortie_produit WHERE idP = :idP AND idBS = :idBS AND dateadd != :dateadd");
                                            $stmt->execute([
                                                ':idP' => $idP,
                                                ':idBS' => $idBS,
                                                ':dateadd' => $date
                                            ]);
                                            $qte_sortie_totale_hors_actuelle = floatval($stmt->fetchColumn() ?? 0);
                                    
                                            // Vérification quantité
                                            if (($qte_sortie_totale_hors_actuelle + $quantity) > $qte_eb) {
                                                $reste = $qte_eb - $qte_sortie_totale_hors_actuelle;
                                                echo json_encode([
                                                    'success' => false,
                                                    'message' => "Quantité saisie invalide : reste à sortir $reste"
                                                ]);
                                                exit;
                                            }
                                            ...
                                            // Mise à jour
                                            $stmt = $gc->getDb()->prepare("UPDATE bon_sortie_produit SET quantite = :quantite, unite = :unite WHERE idBS = :idBS AND idP = :idP AND dateadd = :dateadd");
                                            $stmt->execute([
                                                ':quantite' => $quantity,
                                                ':unite' => $unite,
                                                ':idBS' => $idBS,
                                        ...
                                        case 80:
                                            ...
                                            case 81:
                                                ...
                                                //Générer un achat à partir d'une expression
                                                case 82:
                                                    try {
                                                        $connexion = $gc->getDb();
                                                        $idEB = intval($_POST['idEB']);
                                                
                                                        // Vérifie si un achat existe déjà pour cette expression
                                                        $verif = $connexion->prepare("SELECT COUNT(*) FROM achats WHERE expression_besoin_id = :idEB");
                                                        $verif->execute([':idEB' => $idEB]);
                                                        if ($verif->fetchColumn() > 0) {
                                                            echo json_encode(["message" => "Un achat a déjà été généré pour cette expression."]);
                                                            break;
                                                        }
                                                
                                                        // Vérifie que l'expression existe
                                                        $check = $connexion->prepare("SELECT * FROM expression_besoin WHERE idEB = :idEB");
                                                        $check->execute([':idEB' => $idEB]);
                                                        $expression = $check->fetch(PDO::FETCH_ASSOC);
                                                        if (!$expression) {
                                                            echo json_encode(["message" => "Expression de besoin introuvable."]);
                                                            break;
                                                        }
                                                
                                                        // Insertion de l’achat
                                                        $insert = $connexion->prepare("
                                                            INSERT INTO achats (expression_besoin_id, date_creation, statut)
                                                            VALUES (:idEB, NOW(), 'en_attente')
                                                        ");
                                                        $insert->execute([':idEB' => $idEB]);
                                                
                                                        echo json_encode(["message" => "Achat généré avec succès."]);
                                                    } catch (Exception $e) {
                                                        echo json_encode(["message" => "Erreur serveur : " . $e->getMessage()]);
                                                    }
                                                    break;

                                                // Récupérer les lignes de budget associées à un budget donné avec quantité restante
                                                case 83:
                                                    try {
                                                        $connexion = $gc->getDb();
                                                    ...
                                                    break;                                                                                                
                                                        ...
                                                        case 84:
                                                            try {
                                                                $connexion = $gc->getDb();
                                                                $query = "
                                                                    SELECT b.id, b.annee, tb.nom AS type_budget, b.date_creation, 
                                                                           b.montant_total, b.statut
                                                                    FROM budget b
                                                            ...
                                                            //Récupérer les types de budget
                                                        case 85:
                                                            try {
                                                                $stmt = $gc->getDb()->prepare("SELECT id, nom FROM typebudget ORDER BY nom ASC");
                                                                $stmt->execute();
                                                        ...
                                                        //Charger les infos d’un budget
                                                        case 86:
                                                            try {
                                                                $id = intval($_POST['id']);
                                                                $stmt = $gc->getDb()->prepare("SELECT * FROM budget WHERE id = :id");
                                                                $stmt->execute([':id' => $id]);
                                                        ...
                                                        case 87:
                                                            try {
                                                                ...
                                                                $id = intval($_POST['id']);
                                                                $annee = $_POST['annee'];
                                                                $type_budget_id = intval($_POST['type_budget_id']);
                                                                $montant = floatval($_POST['montant_total']);
                                                                $statut = $_POST['statut'];
                                                        
                                                                $stmt = $db->prepare("UPDATE budget SET annee = :annee, type_budget_id = :type_budget_id, montant_total = :montant, statut = :statut WHERE id = :id");
                                                                $stmt->execute([
                                                                    ':annee' => $annee,
                                                        ...
                                                        case 88:
                                                            try {
                                                                $connexion = $gc->getDb();
                                                                $query = "
                                                                    SELECT a.id, a.date_creation, a.statut, 
                                                                           eb.titre, eb.structure
                                                                    FROM achats a
                                                                    LEFT JOIN expression_besoin eb ON a.expression_besoin_id = eb.idEB
                                                            ...
                                                            //Liste expression de besoin validées
                                                        case 89:
                                                            try {
                                                                $connexion = $gc->getDb();
                                                        
                                                                $query = "
                                                                    SELECT eb.idEB, eb.titre, eb.structure, eb.nom, eb.prenom, eb.date_creation, eb.budget_id
                                                                    FROM expression_besoin eb
                                                                    WHERE eb.Etat_expression_besoin = 1
                                                                    ORDER BY eb.date_creation DESC
                                                                ";
                                                                $stmt = $connexion->prepare($query);
                                                                $stmt->execute();
                                                                ...
                                                                foreach ($expressions as $index => $exp) {
                                                                    ...
                                                                    $stmtTotal = $connexion->prepare("
                                                                        SELECT SUM(prix_unitaire * quantite) AS montant
                                                                        FROM lignebudget
                                                                        WHERE budget_id = :budgetId
                                                                    ");
                                                                    $stmtTotal->execute([':budgetId' => $budgetId]);
                                                                    $montant = $stmtTotal->fetch(PDO::FETCH_ASSOC)['montant'] ?? 0;
                                                        
                                                                    $expressions[$index]['montant_estime'] = number_format($montant, 2, '.', '');
                                                                }
                                                            ...
                                                            //modification d'expression de besoin
                                                            //charger les données
                                                            case 90:
                                                                try {
                                                                    $idEB = intval($_POST['idEB']);
                                                                    $stmt = $gc->getDb()->prepare("SELECT * FROM expression_besoin WHERE idEB = :id");
                                                            ...
                                                            case 91:
                                                                try {
                                                                    $db = $gc->getDb();
                                                                    $stmt = $db->prepare("
                                                                        UPDATE expression_besoin 
                                                                        SET titre = :titre, structure = :structure, nom = :nom, prenom = :prenom 
                                                                        WHERE idEB = :idEB
                                                                    ");
                                                                    $stmt->execute([
                                                                        ':titre' => $_POST['titre'],
                                                                    ...
                                                                    echo json_encode(["success" => true, "message" => "Expression modifiée avec succès."]);
                                                                } catch (Exception $e) {
                                                                    echo json_encode(["success" => false, "message" => "Erreur : " . $e->getMessage()]);
                                                                }
                                                                break;
                                                            
                                                            //Supprimer une ligne de budget d'une expression
                                                            case 92:
                                                                try {
                                                                    $id = intval($_POST['ligne_id']);
                                                                    $stmt = $gc->getDb()->prepare("DELETE FROM lignebudget WHERE id = :id");
                                                                    $stmt->execute([':id' => $id]);
                                                            ...
                                                            // Charger les rubriques disponibles dans un budget
                                                            case 93:
                                                                try {
                                                                    $budget_id = intval($_POST['budget_id']);
                                                                    $stmt = $gc->getDb()->prepare("
                                                            ...
                                                            // Charger les lignes de budget selon rubrique et sous-rubrique
                                                            case 94:
                                                                try {
                                                                    $budget_id = intval($_POST['budget_id']);
                                                                    $rubrique_id = intval($_POST['rubrique_id']);
                                                            
                                                                    $stmt = $gc->getDb()->prepare("
                                                                        SELECT id, designation
                                                                        FROM lignebudget
                                                                        WHERE budget_id = :budget_id AND rubrique_id = :rubrique_id
                                                            ...
                                                            // Ajouter une ligne existante à une expression (rattachement)
                                                            case 95:
                                                                try {
                                                                    $db = $gc->getDb();
                                                                    ...
                                                                    $stmt = $db->prepare("
                                                                        ...
                                                                        WHERE id = :ligne_id AND budget_id = :budget_id
                                                                    ");
                                                                    ...
                                                                    echo json_encode(["message" => "Ligne ajoutée à l'expression avec succès."]);
                                                                } catch (Exception $e) {
                                                                    echo json_encode(["message" => "Erreur : " . $e->getMessage()]);
                                                                }
                                                                break;
                                                                
                                                                //Récupération de plusieurs lignes par ID (pour le PDF)
                                                            case 96:
                                                                try {
                                                                    $connexion = $gc->getDb();
                                                                    $ids = $_POST['ligne_ids'] ?? [];
                                                                    ...
                                                                    $query = "
                                                                        SELECT lb.id, lb.designation, lb.quantite, lb.prix_unitaire, lb.description,
                                                                                lb.prix_reel, lb.quantite_reelle,
                                                                                r.nom AS rubrique, sr.nom AS sous_rubrique
                                                                        FROM lignebudget lb
                                                                        LEFT JOIN rubrique r ON lb.rubrique_id = r.id
                                                                        LEFT JOIN sousrubrique sr ON lb.sous_rubrique_id = sr.id
                                                                ...
                                                                try {
                                                                    ...
                                                                    for ($i = 0; $i < $n; $i++) {
                                                                        ...
                                                                        if ($qte > $reste) {
                                                                            echo json_encode([
                                                                                "success" => false,
                                                                                "message" => "Quantité réelle ($qte) dépasse la quantité restante ($reste) pour la ligne " . ($i + 1)
                                                                            ]);
                                                                            return;
                                                                        }
                                                                    }
